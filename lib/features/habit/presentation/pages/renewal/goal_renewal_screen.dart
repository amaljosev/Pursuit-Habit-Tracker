// presentation/pages/renewal/goal_renewal_screen.dart
      ...
      child: Scaffold(
        ...
        body: BlocConsumer<HabitBloc, HabitState>(
          ...
          builder: (context, state) {
            if (state is HabitError) return const ErrorScreenWidget();
            if (state is HabitLoading) return const AppLoading();
                                  ...
                                  child: Row(
                                    children: [
                                      ...
                                      Expanded(
                                        child: Text(
                                          currentEndDate != null
                                              ? '${_getDayName(currentEndDate.weekday)}, '
                                                  '${_getMonthName(currentEndDate.month)} '
                                                  '${currentEndDate.day}, '
                                                  '${currentEndDate.year}'
                                              : 'No end date set',
                                          style: Theme.of(context)
                        ...
                        Expanded(
                          child: AppButton(
                            ...
                            backgroundColor: HelperFunctions.getColorById(
                                id: habit.color, isDark: true),
                            onPressed: () {
                              // Only update endDate — no stat fields touched
                              final updatedHabit = habit.copyWith(
                                endDate: selectedDate,
                              );
                              context
                                  .read<DetailBloc>()
                                  .add(ResetHabitScreenEvent());
                              context
                                  .read<HabitBloc>()
                                  .add(UpdateHabitEvent(updatedHabit));
                            },
                          ),
                        ),
                      ],
